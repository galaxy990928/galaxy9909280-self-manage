...
import MenuListContent from '@utils/Content/Header/MenuLists';
import MenuList from './MenuList';
import { HeaderWrapper, HeaderWrap, MenuWrap, MenuBarWrap } from './styles';

const Header = (): React.ReactElement => {
  const [isShownSubMenu, setIsShownSubMenu] = useState<boolean[]>([]);
  const onMouseOverMenuList = useCallback(
    (index) => {
      setIsShownSubMenu(
        produce(isShownSubMenu, (draft) => {
          draft[index] = true;
        })
      );
    },
  ...
  const onMouseLeaveMenuList = useCallback(
    (index) => {
      setIsShownSubMenu(
        produce(isShownSubMenu, (draft) => {
          draft[index] = false;
  ...
  return (
    <HeaderWrapper>
      <HeaderWrap>
        ...
        <MenuWrap>
          {MenuListContent.map((menu, index) => (
            <MenuList
              key={menu.id}
              content={menu}
              isShown={isShownSubMenu[index]}
              onMouseOver={() => onMouseOverMenuList(index)}
              onMouseLeave={() => onMouseLeaveMenuList(index)}
            />
          ))}
        </MenuWrap>
        <MenuBarWrap>
          <svg viewBox="0 0 30 30">
            <path d="M2,4v2h26V4H2z M2,16h26v-2H2V16z M2,26h26v-2H2V26z" />
          </svg>
        </MenuBarWrap>
      </HeaderWrap>
    </HeaderWrapper>
